...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="nyb"; }
echo 'nyb() { "$HOME/core-token/packages/core/bin/run" "$@" --token="nyb"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/PhillipJacobsen/nybble_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/PhillipJacobsen/nybble_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@nyb"
rm -rf "$HOME/.config/@Nybble"
rm -rf "$HOME/.config/nyb-core"
